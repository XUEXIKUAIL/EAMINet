...
class ContrastCELoss(nn.Module, ABC):
    ...
    def forward(self, seg, embedding, target, with_embed=False):
        ...
        target = F.interpolate(target.unsqueeze(1).float(), size=(h, w))  # 3-D -》 4-D
        # h, w = target.size(1), target.size(2)
        embedding = F.interpolate(input=embedding, scale_factor=4, mode='bilinear', align_corners=False)
